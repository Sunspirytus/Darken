...
void MainViewPort::InitScene()
{
	...
	std::shared_ptr<Material> SimpleObjectShadowDepthMaterial = _MaterialManager->CreateMaterial(DKEngine::GetInstance().GetAssetFolderPath() + "SimpleObjectShadowDepthMaterial", std::vector<String>{ "ShadowDepthVertShader.vsh", "ShadowDepthFragShader.fsh" }, MaterialBelong::UserDefine);
	std::shared_ptr<MaterialInstance> SimpleObjectShadowDepthMaterialInst = _MaterialManager->CreateMaterialInstance(DKEngine::GetInstance().GetAssetFolderPath() + "SimpleObjectShadowDepthMaterialInst", SimpleObjectShadowDepthMaterial, MaterialBelong::UserDefine);
	...
	//std::shared_ptr<Material> SimpleObjectShadowDepthMaterial2 = std::shared_ptr<Material>(new Material(*SimpleObjectShadowDepthMaterial));
	{
		...
		std::shared_ptr<Material> SimpleObjectMaterial00 = _MaterialManager->CreateMaterial(DKEngine::GetInstance().GetAssetFolderPath() + "SimpleObjectMaterial00", std::vector<String> { "SimpleVertShader.vsh", "SimpleFragShader.fsh" }, MaterialBelong::UserDefine);
		std::shared_ptr<MaterialInstance> SimpleObjectMaterialInst00 = _MaterialManager->CreateMaterialInstance(DKEngine::GetInstance().GetAssetFolderPath() + "SimpleObjectMaterialInst00", SimpleObjectMaterial00, MaterialBelong::UserDefine);
	...
	Scene->AddCamera((uint32)CameraIndex::MainCamera, ViewCamera);
...
{
	Scene->GetCamera((uint32)CameraIndex::MainCamera)->ActiveViewPort();
	RenderPipeline->Render(Scene->GetCamera((uint32)CameraIndex::MainCamera));
	//glFlush();
}

void MainViewPort::OnSizeChange()
{
	std::shared_ptr<Camera> ViewCamera = Scene->GetCamera((uint32)CameraIndex::MainCamera);
