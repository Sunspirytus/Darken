...
void InitSimpleObjects()
{
	...
	ReflectionActor = std::shared_ptr<SphereReflectionCapture>(new SphereReflectionCapture(_Scene, Vector3f(0.0, 0.0, 5.0), 1000.0, 1.0));
...
void CaptureReflection()
{
	...
	for (int32 Index = 0; Index < AbstractActors.size(); Index++)
	{
		...
		if (Capture != nullptr)
		{
			Capture->CaptureWithPipeLine(DeferPipeline.get());
...
void InitView()
{
	...
	std::shared_ptr<LightGroup> Lights = std::shared_ptr<LightGroup>(new LightGroup(_Scene));
	
	DeferPipeline = std::shared_ptr<DeferRenderPipeline>(new DeferRenderPipeline());
	DeferPipeline->Init(_Scene);
...
void Render()
{
	...
	DeferPipeline->Render(_Scene->GetCamera(CameraIndex::MainCamera));*/
	...
	DeferPipeline->RenderLightingPass(_Scene->GetCamera(CameraIndex::MainCamera), StaticMesh | DynamicMesh);
